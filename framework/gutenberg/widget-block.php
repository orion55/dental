...
class Orion_Widgets_Bundle_Widget_Block {
	/**
	...
	public function register_widget_block() {
		register_block_type( 'orion/widget-block', array(
	...
	public function enqueue_widget_block_editor_assets() {
		wp_enqueue_script(
			'orion-widget-block',
			get_template_directory_uri(). '/framework/gutenberg/orion-widgets.js',
		...
		wp_localize_script(
			...
			array(
				...
				'confirmChangeWidget' => __( 'Selecting a different widget will revert any changes. Continue?', 'dentalia' ),
			)
		);
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'sowb-widget-block', 'dentalia' );
		}
	...
	public function render_widget_block( $attributes ) {
		if ( empty( $attributes['widgetClass'] ) ) {
			return '<div>'.
				   __( 'You need to select a widget type before you\'ll see anything here. :)', 'dentalia' ) .
				   '</div>';
		}
		...
		if ( ! empty( $widget ) && is_object( $widget ) && is_subclass_of( $widget, 'SiteOrigin_Widget' ) ) {
			...
			return '<div>'.
				   sprintf(
			   			__( 'Invalid widget class %s. Please make sure the widget has been activated in %sSiteOrigin Widgets%s.', 'dentalia' ),
					   $widget_class,
...
Orion_Widgets_Bundle_Widget_Block::single();
